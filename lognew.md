kalki@kalki-gpu:~/Documents/workspace/Time-Series-Library$ source /home/kalki/Documents/workspace/Time-Series-Library/tsl-env/bin/activate
(tsl-env) kalki@kalki-gpu:~/Documents/workspace/Time-Series-Library$ git fetch
(tsl-env) kalki@kalki-gpu:~/Documents/workspace/Time-Series-Library$ python ./scripts/train/train_celestial_production.py
Could not register with unified registry
WARNING:root:Could not import TSNormalizer from utils.normalization
WARNING:root:Wavelet components not available: attempted relative import beyond top-level package
INFO:__main__:Starting PRODUCTION Celestial Enhanced PGAT training run
INFO:__main__:Heavy-duty overnight configuration enabled
INFO:__main__:================================================================================
INFO:__main__:Memory diagnostics enabled; detailed logs stored at checkpoints/celestial_enhanced_pgat_production_overnight/memory_diagnostics.log
INFO:__main__:Configuration summary | model=Celestial_Enhanced_PGAT seq_len=250 pred_len=10 d_model=416 n_heads=8 e_layers=4 d_layers=2 train_epochs=50
INFO:__main__:Optimization summary | batch_size=16 learning_rate=0.001 patience=999 target=log_Open,log_High,log_Low,log_Close target_wave_indices=[0, 1, 2, 3] c_out=4 wave_aggregation=True
INFO:__main__:Using device: cpu (CPU fallback)
INFO:__main__:Reproducibility seed configured to 2024
INFO:__main__:Loading production data modules
INFO:__main__:Loaded train data module | samples=6750 batches=421 batch_size=16 drop_last=True shuffle=True pin_memory=False prefetch_factor=n/a persistent_workers=False
INFO:__main__:Loaded val data module | samples=41 batches=3 batch_size=16 drop_last=False shuffle=False pin_memory=False prefetch_factor=n/a persistent_workers=False
INFO:__main__:Loaded test data module | samples=41 batches=3 batch_size=16 drop_last=False shuffle=False pin_memory=False prefetch_factor=n/a persistent_workers=False
INFO:__main__:Data loader sizes | train=421 val=3 test=3
INFO:__main__:Preparing scaling utilities for loss computation
INFO:__main__:Main scaler detected with 118 features
INFO:__main__:Target scaler detected with 4 features
INFO:__main__:Using target indices for OHLC: [0, 1, 2, 3]
INFO:__main__:Initializing production Celestial Enhanced PGAT model
🔧 AUTO-CORRECTING INPUT WAVE COUNT:
   Configured: 118
   Detected from CSV: 113
   Using detected value: 113
📊 Analyzing CSV with 119 columns...
📊 Found 113 celestial columns out of 119 total
⚠️  2 features could not be assigned to celestial bodies:
   117: Ascendant_sin
   118: Ascendant_cos
⚠️  chiron not found in CSV
📊 Assigning Ascendant features to Chiron (closest astrological match)

🌌 CELESTIAL FEATURE MAPPING REPORT:
============================================================
🪐 sun          (10 features): [0, 1, 2, 3, 4, 5, 6, 80, 87, 88]
   Features: dyn_Sun_sin, dyn_Sun_cos, dyn_Sun_speed ... Sun_sin, Sun_cos
🪐 moon         (10 features): [7, 8, 9, 10, 11, 12, 13, 81, 89, 90]
   Features: dyn_Moon_sin, dyn_Moon_cos, dyn_Moon_speed ... Moon_sin, Moon_cos
🪐 mercury      (10 features): [21, 22, 23, 24, 25, 26, 27, 83, 93, 94]
   Features: dyn_Mercury_sin, dyn_Mercury_cos, dyn_Mercury_speed ... Mercury_sin, Mercury_cos
🪐 venus        (10 features): [35, 36, 37, 38, 39, 40, 41, 85, 97, 98]
   Features: dyn_Venus_sin, dyn_Venus_cos, dyn_Venus_speed ... Venus_sin, Venus_cos
🪐 mars         (10 features): [14, 15, 16, 17, 18, 19, 20, 82, 91, 92]
   Features: dyn_Mars_sin, dyn_Mars_cos, dyn_Mars_speed ... Mars_sin, Mars_cos
🪐 jupiter      (10 features): [28, 29, 30, 31, 32, 33, 34, 84, 95, 96]
   Features: dyn_Jupiter_sin, dyn_Jupiter_cos, dyn_Jupiter_speed ... Jupiter_sin, Jupiter_cos
🪐 saturn       (10 features): [42, 43, 44, 45, 46, 47, 48, 86, 99, 100]
   Features: dyn_Saturn_sin, dyn_Saturn_cos, dyn_Saturn_speed ... Saturn_sin, Saturn_cos
🪐 uranus       ( 9 features): [49, 50, 51, 52, 53, 54, 55, 101, 102]
   Features: dyn_Uranus_sin, dyn_Uranus_cos, dyn_Uranus_speed ... Uranus_sin, Uranus_cos
🪐 neptune      ( 9 features): [56, 57, 58, 59, 60, 61, 62, 103, 104]
   Features: dyn_Neptune_sin, dyn_Neptune_cos, dyn_Neptune_speed ... Neptune_sin, Neptune_cos
🪐 pluto        ( 9 features): [63, 64, 65, 66, 67, 68, 69, 105, 106]
   Features: dyn_Pluto_sin, dyn_Pluto_cos, dyn_Pluto_speed ... Pluto_sin, Pluto_cos
🪐 north_node   ( 7 features): [70, 71, 72, 73, 74, 107, 108]
   Features: dyn_Mean Rahu_sin, dyn_Mean Rahu_cos, dyn_Mean Rahu_speed ... Mean Rahu_sin, Mean Rahu_cos
🪐 south_node   ( 7 features): [75, 76, 77, 78, 79, 109, 110]
   Features: dyn_Mean Ketu_sin, dyn_Mean Ketu_cos, dyn_Mean Ketu_speed ... Mean Ketu_sin, Mean Ketu_cos
🪐 chiron       ( 2 features): [111, 112]
   Features: Ascendant_sin, Ascendant_cos
============================================================
📊 Total celestial features mapped: 113
📊 Available celestial columns: 113
📊 Total CSV columns: 119
📊 Expected input waves: 113
📊 Max feature index used: 112
✅ Celestial mapping validation complete.

🌌 Phase-Aware Celestial Processor initialized:
   - Input waves: 113 (auto-detected)
   - Celestial bodies: 13
   - Celestial dimension: 32
   - Dynamic feature mapping: ENABLED
   - Assumes phase info already in inputs (sin/cos encoded)
   - Will compute explicit phase differences for edges
🌌 Celestial Wave Aggregator initialized:
   - Input waves: 118
   - Celestial bodies: 13
   - moon: 9 waves
   - mercury: 9 waves
   - venus: 9 waves
   - sun: 9 waves
   - mars: 9 waves
   - jupiter: 9 waves
   - saturn: 9 waves
   - uranus: 9 waves
   - neptune: 9 waves
   - pluto: 9 waves
   - north_node: 12 waves
   - south_node: 8 waves
   - chiron: 3 waves
INFO:models.Celestial_Enhanced_PGAT.Model:Celestial projection preserves: 416D → 416D (1.0x)
INFO:models.Celestial_Enhanced_PGAT.Model:🚀 Initializing Petri Net Combiner with message passing (edge_feature_dim=6, num_steps=2)
INFO:models.Celestial_Enhanced_PGAT.Model:✅ Petri Net Combiner initialized - memory efficient with ZERO information loss!
INFO:models.Celestial_Enhanced_PGAT.Model:🚀 Using EdgeConditionedGraphAttention - ZERO information loss from edge features!
INFO:models.Celestial_Enhanced_PGAT.Model:🎯 Target Autocorrelation Module enabled with 2 layers
INFO:models.Celestial_Enhanced_PGAT.Model:📅 Calendar Effects Module enabled with 104D embeddings
INFO:__main__:Model initialized successfully
INFO:__main__:Parameter statistics | total=44,236,621 trainable=44,236,621 approx_size_mb=168.7
INFO:__main__:Standard precision training
INFO:__main__:Gradient accumulation configured | steps=2 effective_batch_size=32
INFO:__main__:Learning rate schedule preview | warmup_epochs=5 base_lr=0.001000 min_lr=0.000001 remaining_epochs=45
INFO:__main__:Sample learning rates: E1:0.000200, E5:0.001000, E6:0.001000, E26:0.000587, E50:0.000002
INFO:__main__:Using MSE Loss for deterministic predictions
INFO:__main__:Early stopping patience configured to 999 (effectively disabled)
INFO:__main__:Checkpoints will be saved to checkpoints/celestial_enhanced_pgat_production_overnight
INFO:__main__:Inspecting data file at ./data/prepared_financial_data.csv for target resolution
INFO:__main__:Feature summary | total_columns=119 feature_columns=118
INFO:__main__:Estimated CSV row count: 7109
INFO:__main__:Auto-set enc_in/dec_in to 118 based on feature count
INFO:__main__:Resolved target indices: [0, 1, 2, 3]
INFO:__main__:Resolved target names: ['log_Open', 'log_High', 'log_Low', 'log_Close']
INFO:__main__:Starting production training loop
INFO:__main__:Configured for 50 training epochs
INFO:__main__:Epoch 1/50 - production training
🔍 MEMORY [FORWARD_START] Input shapes: x_enc=torch.Size([16, 250, 118]), x_dec=torch.Size([16, 135, 118])
   CPU Memory: 0.75GB
🔍 MEMORY [BEFORE_ENC_EMBEDDING] 
   CPU Memory: 1.05GB
🔍 MEMORY [AFTER_ENC_EMBEDDING] enc_out shape: torch.Size([16, 250, 416])
   CPU Memory: 1.11GB
🔍 MEMORY [BEFORE_MARKET_CONTEXT] 
   CPU Memory: 1.13GB
🔍 MEMORY [AFTER_MARKET_CONTEXT] market_context shape: torch.Size([16, 250, 416])
   CPU Memory: 1.15GB
🔍 MEMORY [BEFORE_ADJ_WEIGHT_MLP] market_context: torch.Size([16, 250, 416])
   CPU Memory: 4.68GB
🔍 MEMORY [AFTER_ADJ_WEIGHT_MLP] weights: torch.Size([16, 250, 3])
   CPU Memory: 4.68GB
🔍 MEMORY [BEFORE_ADJ_NORMALIZE] 
   CPU Memory: 4.68GB
🔍 MEMORY [AFTER_ADJ_NORMALIZE] phase_norm: torch.Size([16, 250, 13, 13]), astro_norm: torch.Size([16, 250, 13, 13])
   CPU Memory: 4.68GB
🔍 MEMORY [BEFORE_ADJ_FUSION] Broadcasting torch.Size([16, 250, 1, 1]) with torch.Size([16, 250, 13, 13])
   CPU Memory: 4.68GB
🔍 MEMORY [AFTER_ADJ_FUSION] combined_phase_adj: torch.Size([16, 250, 13, 13])
   CPU Memory: 4.68GB
🔍 MEMORY BEFORE celestial_combiner: CPU=4.68GB
🔍 INPUT SHAPES to celestial_combiner:
   astronomical_adj: torch.Size([16, 250, 13, 13])
   learned_adj: torch.Size([16, 250, 13, 13])
   dynamic_adj: torch.Size([16, 250, 13, 13])
   enc_out: torch.Size([16, 250, 416])
🚀 PETRI NET: Rich edge features preserved!
   combined_adj: torch.Size([16, 250, 13, 13])
   rich_edge_features: torch.Size([16, 250, 13, 13, 6]) (6D vectors, NO compression!)
   Edge features: [theta_diff, phi_diff, velocity_diff, radius_ratio, long_diff, phase_alignment]
✅ MEMORY AFTER celestial_combiner: CPU=13.06GB
📊 MEMORY DELTA: CPU=+8.38GB
✅ OUTPUT SHAPES from celestial_combiner:
   combined_adj: torch.Size([16, 250, 13, 13])
   fusion_metadata keys: ['astronomical_contribution', 'learned_contribution', 'attention_contribution', 'edge_feature_dim', 'num_message_passing_steps', 'avg_transition_strength', 'final_edge_density', 'temporal_edge_variance', 'spatial_edge_variance', 'node_state_norm', 'rich_edge_features_shape', 'edge_features_preserved', 'no_compression']
🧹 AFTER GC: CPU=13.06GB
🔍 STEP 6: Hierarchical mapping check (enabled=False)
🔍 STEP 7: Spatiotemporal encoding
   enc_out shape: torch.Size([16, 250, 416])
   combined_adj shape: torch.Size([16, 250, 13, 13])
   ⏭️  Petri bypass enabled — using encoder output directly for graph processing
🔍 STEP 8: Graph attention processing
   encoded_features shape: torch.Size([16, 250, 416])
   use_efficient_covariate_interaction: True
INFO:__main__:Batch 0/421 | loss=1.283732 elapsed=10.1s
🔍 MEMORY [FORWARD_START] Input shapes: x_enc=torch.Size([16, 250, 118]), x_dec=torch.Size([16, 135, 118])
   CPU Memory: 11.80GB
🔍 MEMORY [BEFORE_ENC_EMBEDDING] 
   CPU Memory: 11.80GB
🔍 MEMORY [AFTER_ENC_EMBEDDING] enc_out shape: torch.Size([16, 250, 416])
   CPU Memory: 11.80GB
🔍 MEMORY [BEFORE_MARKET_CONTEXT] 
   CPU Memory: 11.80GB
🔍 MEMORY [AFTER_MARKET_CONTEXT] market_context shape: torch.Size([16, 250, 416])
   CPU Memory: 11.80GB
🔍 MEMORY [BEFORE_ADJ_WEIGHT_MLP] market_context: torch.Size([16, 250, 416])
   CPU Memory: 11.80GB
🔍 MEMORY [AFTER_ADJ_WEIGHT_MLP] weights: torch.Size([16, 250, 3])
   CPU Memory: 11.80GB
🔍 MEMORY [BEFORE_ADJ_NORMALIZE] 
   CPU Memory: 11.80GB
🔍 MEMORY [AFTER_ADJ_NORMALIZE] phase_norm: torch.Size([16, 250, 13, 13]), astro_norm: torch.Size([16, 250, 13, 13])
   CPU Memory: 11.80GB
🔍 MEMORY [BEFORE_ADJ_FUSION] Broadcasting torch.Size([16, 250, 1, 1]) with torch.Size([16, 250, 13, 13])
   CPU Memory: 11.80GB
🔍 MEMORY [AFTER_ADJ_FUSION] combined_phase_adj: torch.Size([16, 250, 13, 13])
   CPU Memory: 11.80GB
🔍 MEMORY BEFORE celestial_combiner: CPU=11.80GB
🔍 INPUT SHAPES to celestial_combiner:
   astronomical_adj: torch.Size([16, 250, 13, 13])
   learned_adj: torch.Size([16, 250, 13, 13])
   dynamic_adj: torch.Size([16, 250, 13, 13])
   enc_out: torch.Size([16, 250, 416])
🚀 PETRI NET: Rich edge features preserved!
   combined_adj: torch.Size([16, 250, 13, 13])
   rich_edge_features: torch.Size([16, 250, 13, 13, 6]) (6D vectors, NO compression!)
   Edge features: [theta_diff, phi_diff, velocity_diff, radius_ratio, long_diff, phase_alignment]
✅ MEMORY AFTER celestial_combiner: CPU=18.29GB
📊 MEMORY DELTA: CPU=+6.49GB
✅ OUTPUT SHAPES from celestial_combiner:
   combined_adj: torch.Size([16, 250, 13, 13])
   fusion_metadata keys: ['astronomical_contribution', 'learned_contribution', 'attention_contribution', 'edge_feature_dim', 'num_message_passing_steps', 'avg_transition_strength', 'final_edge_density', 'temporal_edge_variance', 'spatial_edge_variance', 'node_state_norm', 'rich_edge_features_shape', 'edge_features_preserved', 'no_compression']
🧹 AFTER GC: CPU=18.29GB
🔍 STEP 6: Hierarchical mapping check (enabled=False)
🔍 STEP 7: Spatiotemporal encoding
   enc_out shape: torch.Size([16, 250, 416])
   combined_adj shape: torch.Size([16, 250, 13, 13])
   ⏭️  Petri bypass enabled — using encoder output directly for graph processing
🔍 STEP 8: Graph attention processing
   encoded_features shape: torch.Size([16, 250, 416])
   use_efficient_covariate_interaction: True
🔍 MEMORY [FORWARD_START] Input shapes: x_enc=torch.Size([16, 250, 118]), x_dec=torch.Size([16, 135, 118])
   CPU Memory: 18.30GB
🔍 MEMORY [BEFORE_ENC_EMBEDDING] 
   CPU Memory: 18.30GB
🔍 MEMORY [AFTER_ENC_EMBEDDING] enc_out shape: torch.Size([16, 250, 416])
   CPU Memory: 18.30GB
🔍 MEMORY [BEFORE_MARKET_CONTEXT] 
   CPU Memory: 18.30GB
🔍 MEMORY [AFTER_MARKET_CONTEXT] market_context shape: torch.Size([16, 250, 416])
   CPU Memory: 18.30GB
🔍 MEMORY [BEFORE_ADJ_WEIGHT_MLP] market_context: torch.Size([16, 250, 416])
   CPU Memory: 18.30GB
🔍 MEMORY [AFTER_ADJ_WEIGHT_MLP] weights: torch.Size([16, 250, 3])
   CPU Memory: 18.30GB
🔍 MEMORY [BEFORE_ADJ_NORMALIZE] 
   CPU Memory: 18.30GB
🔍 MEMORY [AFTER_ADJ_NORMALIZE] phase_norm: torch.Size([16, 250, 13, 13]), astro_norm: torch.Size([16, 250, 13, 13])
   CPU Memory: 18.30GB
🔍 MEMORY [BEFORE_ADJ_FUSION] Broadcasting torch.Size([16, 250, 1, 1]) with torch.Size([16, 250, 13, 13])
   CPU Memory: 18.30GB
🔍 MEMORY [AFTER_ADJ_FUSION] combined_phase_adj: torch.Size([16, 250, 13, 13])
   CPU Memory: 18.30GB
🔍 MEMORY BEFORE celestial_combiner: CPU=18.30GB
🔍 INPUT SHAPES to celestial_combiner:
   astronomical_adj: torch.Size([16, 250, 13, 13])
   learned_adj: torch.Size([16, 250, 13, 13])
   dynamic_adj: torch.Size([16, 250, 13, 13])
   enc_out: torch.Size([16, 250, 416])
🚀 PETRI NET: Rich edge features preserved!
   combined_adj: torch.Size([16, 250, 13, 13])
   rich_edge_features: torch.Size([16, 250, 13, 13, 6]) (6D vectors, NO compression!)
   Edge features: [theta_diff, phi_diff, velocity_diff, radius_ratio, long_diff, phase_alignment]
✅ MEMORY AFTER celestial_combiner: CPU=23.08GB
📊 MEMORY DELTA: CPU=+4.79GB
✅ OUTPUT SHAPES from celestial_combiner:
   combined_adj: torch.Size([16, 250, 13, 13])
   fusion_metadata keys: ['astronomical_contribution', 'learned_contribution', 'attention_contribution', 'edge_feature_dim', 'num_message_passing_steps', 'avg_transition_strength', 'final_edge_density', 'temporal_edge_variance', 'spatial_edge_variance', 'node_state_norm', 'rich_edge_features_shape', 'edge_features_preserved', 'no_compression']
🧹 AFTER GC: CPU=23.08GB
🔍 STEP 6: Hierarchical mapping check (enabled=False)
🔍 STEP 7: Spatiotemporal encoding
   enc_out shape: torch.Size([16, 250, 416])
   combined_adj shape: torch.Size([16, 250, 13, 13])
   ⏭️  Petri bypass enabled — using encoder output directly for graph processing
🔍 STEP 8: Graph attention processing
   encoded_features shape: torch.Size([16, 250, 416])
   use_efficient_covariate_interaction: True
🔍 MEMORY [FORWARD_START] Input shapes: x_enc=torch.Size([16, 250, 118]), x_dec=torch.Size([16, 135, 118])
   CPU Memory: 18.27GB
🔍 MEMORY [BEFORE_ENC_EMBEDDING] 
   CPU Memory: 18.27GB
🔍 MEMORY [AFTER_ENC_EMBEDDING] enc_out shape: torch.Size([16, 250, 416])
   CPU Memory: 18.27GB
🔍 MEMORY [BEFORE_MARKET_CONTEXT] 
   CPU Memory: 18.27GB
🔍 MEMORY [AFTER_MARKET_CONTEXT] market_context shape: torch.Size([16, 250, 416])
   CPU Memory: 18.27GB
🔍 MEMORY [BEFORE_ADJ_WEIGHT_MLP] market_context: torch.Size([16, 250, 416])
   CPU Memory: 18.27GB
🔍 MEMORY [AFTER_ADJ_WEIGHT_MLP] weights: torch.Size([16, 250, 3])
   CPU Memory: 18.27GB
🔍 MEMORY [BEFORE_ADJ_NORMALIZE] 
   CPU Memory: 18.27GB
🔍 MEMORY [AFTER_ADJ_NORMALIZE] phase_norm: torch.Size([16, 250, 13, 13]), astro_norm: torch.Size([16, 250, 13, 13])
   CPU Memory: 18.27GB
🔍 MEMORY [BEFORE_ADJ_FUSION] Broadcasting torch.Size([16, 250, 1, 1]) with torch.Size([16, 250, 13, 13])
   CPU Memory: 18.27GB
🔍 MEMORY [AFTER_ADJ_FUSION] combined_phase_adj: torch.Size([16, 250, 13, 13])
   CPU Memory: 18.27GB
🔍 MEMORY BEFORE celestial_combiner: CPU=18.27GB
🔍 INPUT SHAPES to celestial_combiner:
   astronomical_adj: torch.Size([16, 250, 13, 13])
   learned_adj: torch.Size([16, 250, 13, 13])
   dynamic_adj: torch.Size([16, 250, 13, 13])
   enc_out: torch.Size([16, 250, 416])
🚀 PETRI NET: Rich edge features preserved!
   combined_adj: torch.Size([16, 250, 13, 13])
   rich_edge_features: torch.Size([16, 250, 13, 13, 6]) (6D vectors, NO compression!)
   Edge features: [theta_diff, phi_diff, velocity_diff, radius_ratio, long_diff, phase_alignment]
✅ MEMORY AFTER celestial_combiner: CPU=23.21GB
📊 MEMORY DELTA: CPU=+4.94GB
✅ OUTPUT SHAPES from celestial_combiner:
   combined_adj: torch.Size([16, 250, 13, 13])
   fusion_metadata keys: ['astronomical_contribution', 'learned_contribution', 'attention_contribution', 'edge_feature_dim', 'num_message_passing_steps', 'avg_transition_strength', 'final_edge_density', 'temporal_edge_variance', 'spatial_edge_variance', 'node_state_norm', 'rich_edge_features_shape', 'edge_features_preserved', 'no_compression']
🧹 AFTER GC: CPU=23.21GB
🔍 STEP 6: Hierarchical mapping check (enabled=False)
🔍 STEP 7: Spatiotemporal encoding
   enc_out shape: torch.Size([16, 250, 416])
   combined_adj shape: torch.Size([16, 250, 13, 13])
   ⏭️  Petri bypass enabled — using encoder output directly for graph processing
🔍 STEP 8: Graph attention processing
   encoded_features shape: torch.Size([16, 250, 416])
   use_efficient_covariate_interaction: True
🔍 MEMORY [FORWARD_START] Input shapes: x_enc=torch.Size([16, 250, 118]), x_dec=torch.Size([16, 135, 118])
   CPU Memory: 23.21GB
🔍 MEMORY [BEFORE_ENC_EMBEDDING] 
   CPU Memory: 23.21GB
🔍 MEMORY [AFTER_ENC_EMBEDDING] enc_out shape: torch.Size([16, 250, 416])
   CPU Memory: 23.21GB
🔍 MEMORY [BEFORE_MARKET_CONTEXT] 
   CPU Memory: 23.21GB
🔍 MEMORY [AFTER_MARKET_CONTEXT] market_context shape: torch.Size([16, 250, 416])
   CPU Memory: 23.21GB
🔍 MEMORY [BEFORE_ADJ_WEIGHT_MLP] market_context: torch.Size([16, 250, 416])
   CPU Memory: 23.21GB
🔍 MEMORY [AFTER_ADJ_WEIGHT_MLP] weights: torch.Size([16, 250, 3])
   CPU Memory: 23.21GB
🔍 MEMORY [BEFORE_ADJ_NORMALIZE] 
   CPU Memory: 23.21GB
🔍 MEMORY [AFTER_ADJ_NORMALIZE] phase_norm: torch.Size([16, 250, 13, 13]), astro_norm: torch.Size([16, 250, 13, 13])
   CPU Memory: 23.21GB
🔍 MEMORY [BEFORE_ADJ_FUSION] Broadcasting torch.Size([16, 250, 1, 1]) with torch.Size([16, 250, 13, 13])
   CPU Memory: 23.21GB
🔍 MEMORY [AFTER_ADJ_FUSION] combined_phase_adj: torch.Size([16, 250, 13, 13])
   CPU Memory: 23.21GB
🔍 MEMORY BEFORE celestial_combiner: CPU=23.21GB
🔍 INPUT SHAPES to celestial_combiner:
   astronomical_adj: torch.Size([16, 250, 13, 13])
   learned_adj: torch.Size([16, 250, 13, 13])
   dynamic_adj: torch.Size([16, 250, 13, 13])
   enc_out: torch.Size([16, 250, 416])
^A🚀 PETRI NET: Rich edge features preserved!
   combined_adj: torch.Size([16, 250, 13, 13])
   rich_edge_features: torch.Size([16, 250, 13, 13, 6]) (6D vectors, NO compression!)
   Edge features: [theta_diff, phi_diff, velocity_diff, radius_ratio, long_diff, phase_alignment]
✅ MEMORY AFTER celestial_combiner: CPU=27.31GB
📊 MEMORY DELTA: CPU=+4.10GB
✅ OUTPUT SHAPES from celestial_combiner:
   combined_adj: torch.Size([16, 250, 13, 13])
   fusion_metadata keys: ['astronomical_contribution', 'learned_contribution', 'attention_contribution', 'edge_feature_dim', 'num_message_passing_steps', 'avg_transition_strength', 'final_edge_density', 'temporal_edge_variance', 'spatial_edge_variance', 'node_state_norm', 'rich_edge_features_shape', 'edge_features_preserved', 'no_compression']
🧹 AFTER GC: CPU=27.31GB
🔍 STEP 6: Hierarchical mapping check (enabled=False)
🔍 STEP 7: Spatiotemporal encoding
   enc_out shape: torch.Size([16, 250, 416])
   combined_adj shape: torch.Size([16, 250, 13, 13])
   ⏭️  Petri bypass enabled — using encoder output directly for graph processing
🔍 STEP 8: Graph attention processing
   encoded_features shape: torch.Size([16, 250, 416])
   use_efficient_covariate_interaction: True
🔍 MEMORY [FORWARD_START] Input shapes: x_enc=torch.Size([16, 250, 118]), x_dec=torch.Size([16, 135, 118])
   CPU Memory: 27.15GB
🔍 MEMORY [BEFORE_ENC_EMBEDDING] 
   CPU Memory: 27.15GB
🔍 MEMORY [AFTER_ENC_EMBEDDING] enc_out shape: torch.Size([16, 250, 416])
   CPU Memory: 27.15GB
🔍 MEMORY [BEFORE_MARKET_CONTEXT] 
   CPU Memory: 27.15GB
🔍 MEMORY [AFTER_MARKET_CONTEXT] market_context shape: torch.Size([16, 250, 416])
   CPU Memory: 27.15GB
🔍 MEMORY [BEFORE_ADJ_WEIGHT_MLP] market_context: torch.Size([16, 250, 416])
   CPU Memory: 27.15GB
🔍 MEMORY [AFTER_ADJ_WEIGHT_MLP] weights: torch.Size([16, 250, 3])
   CPU Memory: 27.15GB
🔍 MEMORY [BEFORE_ADJ_NORMALIZE] 
   CPU Memory: 27.15GB
🔍 MEMORY [AFTER_ADJ_NORMALIZE] phase_norm: torch.Size([16, 250, 13, 13]), astro_norm: torch.Size([16, 250, 13, 13])
   CPU Memory: 27.15GB
🔍 MEMORY [BEFORE_ADJ_FUSION] Broadcasting torch.Size([16, 250, 1, 1]) with torch.Size([16, 250, 13, 13])
   CPU Memory: 27.15GB
🔍 MEMORY [AFTER_ADJ_FUSION] combined_phase_adj: torch.Size([16, 250, 13, 13])
   CPU Memory: 27.15GB
🔍 MEMORY BEFORE celestial_combiner: CPU=27.15GB
🔍 INPUT SHAPES to celestial_combiner:
   astronomical_adj: torch.Size([16, 250, 13, 13])
   learned_adj: torch.Size([16, 250, 13, 13])
   dynamic_adj: torch.Size([16, 250, 13, 13])
   enc_out: torch.Size([16, 250, 416])
