...
def run_test_suite():
    ...
    print("TEST Running Autoformer Model Test Suite")
    print("=" * 50)
    ...
    for test_file in test_files:
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"PASS {test_file} PASSED ({duration:.2f}s)")
                results[test_file] = "PASSED"
            else:
                print(f"FAIL {test_file} FAILED ({duration:.2f}s)")
    ...
    for test_file, result in results.items():
        status_emoji = {"PASSED": "PASS", "FAILED": "FAIL", "ERROR": ""}[result]
    ...
    if failed == 0 and errors == 0:
        print("\nPARTY ALL TESTS PASSED! Models are ready for use.")
        return True
    else:
        print(f"\nWARN  {failed + errors} test(s) failed. Please check the issues above.")
        return False

def quick_smoke_test():
    """Quick smoke test to verify basic functionality"""
    print("FIRE Running Quick Smoke Test...")
    ...
    try:
        ...
        print("PASS AutoformerFixed smoke test passed")
        ...
        print("PASS EnhancedAutoformer smoke test passed")
        
        print("PARTY Smoke test completed successfully!")
        return True
...
if __name__ == "__main__":
    print("ROCKET Starting Autoformer Test Suite")
    
    # Run quick smoke test first
    if not quick_smoke_test():
        print("FAIL Smoke test failed. Exiting.")
        sys.exit(1)
    ...
    if success:
        print("\nTARGET CONCLUSION: Both models are working perfectly!")
        sys.exit(0)
    else:
        print("\nWARN  CONCLUSION: Some tests failed. Please review and fix.")
        sys.exit(1)