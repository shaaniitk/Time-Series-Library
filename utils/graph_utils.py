    ...
    if isinstance(graph_data, torch.Tensor):
        ...
        if graph_data.dim() == 2:
            ...
            if nodes1 == expected_nodes and nodes2 == expected_nodes:
                return graph_data  # Return full batch
            else:
                # Resize all batches
                device = graph_data.device
            ...
            return torch.eye(expected_nodes, device=graph_data.device)
    
    elif isinstance(graph_data, HeteroData):
        return convert_hetero_to_dense_adj(graph_data, expected_nodes)
    
    elif isinstance(graph_data, dict):
        # Handle dictionary format (edge weights, etc.)
        device = torch.device('cpu')
        if 'adjacency' in graph_data:
            return ensure_tensor_graph_format(graph_data['adjacency'], expected_nodes)
        else:
            # Create identity matrix as fallback
            return torch.eye(expected_nodes, device=device)
    
    else:
        # Unknown format, return identity
        return torch.eye(expected_nodes)
...
def adjacency_to_edge_indices(
    ...
    """
    Convert learned adjacency matrix back to edge indices for graph attention.
    
    Args:
        adjacency_matrix: Learned adjacency [batch_size, total_nodes, total_nodes] or [total_nodes, total_nodes]
    ...
    Returns:
        Dictionary of edge indices for heterogeneous graph attention
    """
    # Handle batch dimension
    if adjacency_matrix.dim() == 3:
        # Use first batch sample for now (could be improved to handle per-sample)
        adj = adjacency_matrix[0]
    else:
    ...
    if adj.shape[0] != total_nodes or adj.shape[1] != total_nodes:
        raise ValueError(f"Adjacency matrix shape {adj.shape} doesn't match expected total nodes {total_nodes}")
    
    # Define node ranges
    wave_start, wave_end = 0, wave_nodes
    transition_start, transition_end = wave_nodes, wave_nodes + transition_nodes
    target_start, target_end = wave_nodes + transition_nodes, wave_nodes + transition_nodes + target_nodes
    
    # Extract wave -> transition edges (preserve learned structure exactly)
    wave_to_transition = adj[wave_start:wave_end, transition_start:transition_end]
    wave_trans_edges = (wave_to_transition > threshold).nonzero(as_tuple=False).t()
    
    # Validate that indices are in expected ranges (they should be by construction)
    if wave_trans_edges.numel() > 0:
        max_wave_idx = wave_trans_edges[0].max().item()
        max_trans_idx = wave_trans_edges[1].max().item()
        if max_wave_idx >= wave_nodes or max_trans_idx >= transition_nodes:
            raise RuntimeError(f"Invalid wave->transition indices: max_wave={max_wave_idx} (limit={wave_nodes-1}), "
                             f"max_trans={max_trans_idx} (limit={transition_nodes-1})")
    else:
        # Fallback: create minimal connectivity to prevent graph attention failure
        wave_trans_edges = torch.tensor([[0], [0]], device=device)
    
    # Extract transition -> target edges (preserve learned structure exactly)
    transition_to_target = adj[transition_start:transition_end, target_start:target_end]
    trans_target_edges = (transition_to_target > threshold).nonzero(as_tuple=False).t()
    
    # Validate that indices are in expected ranges
    if trans_target_edges.numel() > 0:
        max_trans_idx = trans_target_edges[0].max().item()
        max_target_idx = trans_target_edges[1].max().item()
        if max_trans_idx >= transition_nodes or max_target_idx >= target_nodes:
            raise RuntimeError(f"Invalid transition->target indices: max_trans={max_trans_idx} (limit={transition_nodes-1}), "
                             f"max_target={max_target_idx} (limit={target_nodes-1})")
    else:
        # Fallback: create minimal connectivity
        trans_target_edges = torch.tensor([[0], [0]], device=device)
    
    # Convert to expected format (flip for graph attention convention)
    edge_index_dict = {
...
def calculate_topology_features(data: HeteroData) -> dict:
    ...
    clustering_coeffs: Dict[int, float] = nx.clustering(G)
