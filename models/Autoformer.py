...
class EnhancedAutoformer(nn.Module):
    """
    Enhanced Autoformer with learnable series decomposition and improved encoder/decoder layers.
    ...
    1. LearnableSeriesDecomp for improved trend-seasonal decomposition
    2. Enhanced encoder/decoder layers with gated mechanisms
    3. Improved attention mechanisms and layer scaling
    4. Gradient scaling and stable operations
    5. Input validation and configuration checking
    """
    
    def __init__(self, configs):
        super(EnhancedAutoformer, self).__init__()
        
        # Configuration options for various enhancements
        self.use_stable_decomp = getattr(configs, 'use_stable_decomp', False)  # Fixed variant
        self.use_input_validation = getattr(configs, 'use_input_validation', False)  # Improved variant  
        self.use_gradient_scaling = getattr(configs, 'use_gradient_scaling', False)  # Fixed variant
        
        logger.info(f"Initializing EnhancedAutoformer with configs: {configs}")
        logger.info(f"Stability options - stable_decomp: {self.use_stable_decomp}, "
                   f"validation: {self.use_input_validation}, gradient_scaling: {self.use_gradient_scaling}")
...
# Backward compatibility: expose original symbol names expected by other modules
# Some modules import from models.EnhancedAutoformer expecting these classes
EnhancedEncoderLayerExport = EnhancedEncoderLayer
EnhancedDecoderLayerExport = EnhancedDecoderLayer
EnhancedEncoderExport = EnhancedEncoder
EnhancedDecoderExport = EnhancedDecoder

# Some tests and scripts may import StableSeriesDecomp from EnhancedAutoformer_Fixed
# Provide a thin alias if StableSeriesDecomp was defined elsewhere; otherwise fall back to LearnableSeriesDecomp
try:
    from .EnhancedAutoformer_Fixed import StableSeriesDecomp as StableSeriesDecomp  # type: ignore
