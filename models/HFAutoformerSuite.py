"""
Complete Hugging Face Autoformer Suite (Modularized)
"""
...
class HFBayesianAutoformer(HFEnhancedAutoformer):
...
class HFHierarchicalAutoformer(HFEnhancedAutoformer):
    def __init__(self, configs, hierarchy_levels=3):
        super().__init__(configs)
        self.hierarchy_levels = hierarchy_levels
        self.resolution_models = nn.ModuleList([HFEnhancedAutoformer(configs) for _ in range(hierarchy_levels)])
    ...
    def forward(self, x_enc, x_mark_enc, x_dec, x_mark_dec, mask=None):
        ...
        for level, model in enumerate(self.resolution_models):
            ...
            x_down = self.downsample_layers[level](x_down).transpose(1, 2)
            
            mark_down = x_mark_enc.transpose(1, 2)
            mark_down = self.downsample_layers[level](mark_down).transpose(1, 2) if x_mark_enc is not None else None
...
class HFQuantileAutoformer(HFBayesianAutoformer):
