...
def register_example_components():
    """Register example components with the unified registry"""
    
    # Register backbones
    unified_registry.register(ComponentFamily.BACKBONE, 'chronos', ChronosBackbone, metadata={
    ...
    unified_registry.register(ComponentFamily.BACKBONE, 'chronos_x', ChronosXBackbone, metadata={
    ...
    unified_registry.register(ComponentFamily.BACKBONE, 'chronos_x_tiny', ChronosXTinyBackbone, metadata={
    ...
    unified_registry.register(ComponentFamily.BACKBONE, 'chronos_x_large', ChronosXLargeBackbone, metadata={
    ...
    unified_registry.register(ComponentFamily.BACKBONE, 'chronos_x_uncertainty', ChronosXUncertaintyBackbone, metadata={
    ...
    # Register processors
    unified_registry.register(ComponentFamily.PROCESSOR, 'time_domain', TimeDomainProcessor, metadata={
        'description': 'Time domain processing strategy',
        'compatible_attention': ['multi_head', 'sparse']
    })
    
    unified_registry.register(ComponentFamily.PROCESSOR, 'frequency_domain', FrequencyDomainProcessor, metadata={
        'description': 'Frequency domain processing strategy',
        'compatible_attention': ['autocorr', 'fourier']
    })
    
    # Register attention mechanisms
    unified_registry.register(ComponentFamily.ATTENTION, 'multi_head', MultiHeadAttention, metadata={
        'description': 'Standard multi-head attention',
        'compatible_processors': ['time_domain']
    })
    
    unified_registry.register(ComponentFamily.ATTENTION, 'autocorr', AutoCorrAttention, metadata={
    ...
    unified_registry.register(ComponentFamily.LOSS, 'bayesian_mse', BayesianMSELoss, metadata={
        'description': 'Bayesian MSE loss with KL divergence',
        'requires_bayesian_model': True
    })
    
    # Register embeddings
    unified_registry.register(ComponentFamily.EMBEDDING, 'standard', StandardEmbedding, metadata={
        'description': 'Standard embedding with positional encoding'
    })
    
    # Register output components
    unified_registry.register(ComponentFamily.OUTPUT, 'linear', LinearOutput, metadata={
        'description': 'Simple linear output projection',
        'supports_regression': True
    })
    
    unified_registry.register(ComponentFamily.OUTPUT, 'multi_output', MultiOutputHead, metadata={
