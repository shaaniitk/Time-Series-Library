...
class FourierCrossAttention(BaseAttention):
    ...
    def __init__(self, d_model: int, n_heads: int, seq_len_q: int, seq_len_kv: int, dropout: float = 0.1, **kwargs):
        ...
        self.weights_q = nn.Parameter(torch.rand(1, n_heads, 1, self.num_freqs_q, d_model // n_heads, dtype=torch.cfloat))
        self.weights_kv = nn.Parameter(torch.rand(1, n_heads, 1, self.num_freqs_kv, d_model // n_heads, dtype=torch.cfloat))
    ...
    def forward(
        ...
        **kwargs
        ...
        q_filtered_ft = q_ft * self.weights_q
        k_filtered_ft = k_ft * self.weights_kv
