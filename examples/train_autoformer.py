...
def prepare_data(df, seq_len, label_len, pred_len, n_targets, n_covariates):
    ...
    for i in range(len(data) - seq_len - pred_len + 1):
        ...
        # Decoder inputs
        # For the decoder, we need the last `label_len` of the encoder sequence,
        # plus a placeholder for the prediction sequence.
        ...
        # For this example, we'll use dummy time features (marks)
...
def main():
    logger.info("--- Training Standard Autoformer Example ---")
    ...
    for epoch in range(epochs):
        ...
        for i in range(0, len(x_enc), batch_size):
            ...
            # The model outputs the full prediction length, so we match it with the target
